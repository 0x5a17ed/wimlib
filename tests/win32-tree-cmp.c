...
static void
cmp_reparse_data(HANDLE hFile_1,
                 const wchar_t *path_1,
                 HANDLE hFile_2,
                 const wchar_t *path_2)
{
         ...
         u64 size)
{
	...
	while (bytes_remaining) {
		...
		if (memcmp(buf_1, buf_2, bytesToRead))
			difference(
				L"Data of %ls and %ls differs", path_1, path_2);
		bytes_remaining -= bytesToRead;
	}
}

static void
cmp_stream(wchar_t *path_1,
           size_t path_1_len,
           WIN32_FIND_STREAM_DATA *dat_1,
           wchar_t *path_2,
           size_t path_2_len,
           WIN32_FIND_STREAM_DATA *dat_2)
{
	...
	if (wcscmp(dat_1->cStreamName, dat_2->cStreamName)) {
		difference(
			L"Data streams %ls%ls and %ls%ls are not named the same",
			path_1,
			dat_1->cStreamName,
			path_2,
			dat_2->cStreamName);
	...
	if (dat_1->StreamSize.QuadPart != dat_2->StreamSize.QuadPart) {
		difference(L"Data streams %ls%ls (%" PRIu64
		           " bytes) and %ls%ls "
	...
	if (!stream_name) {
		fwprintf(stderr,
		         L"WARNING: unrecognized stream name format %ls\n",
		         dat_1->cStreamName);
		return;
	}
            ...
            size_t path_2_len)
{
	...
	if (nstreams_1 != nstreams_2) {
		difference(
			L"%ls and %ls do not have the same number of streams "
			"(%lu vs %lu)",
			path_1,
			path_2,
			nstreams_1,
...
static void
recurse_directory(wchar_t *path_1,
                  size_t path_1_len,
                  wchar_t *path_2,
                  size_t path_2_len)
{
	...
	for (i = 0; i < ndentries_1; i++) {
		...
		wmemcpy(&path_1[path_1_len + 1],
		        dentries_1[i].cFileName,
		        name_1_len + 1);
		wmemcpy(&path_2[path_2_len + 1],
		        dentries_2[i].cFileName,
		        name_2_len + 1);

		if (wcscmp(dentries_1[i].cFileName, dentries_2[i].cFileName))
		...
		if (wcscmp(dentries_1[i].cAlternateFileName,
			...
			difference(
				L"%ls and %ls do not have the same short name "
				"(%ls vs. %ls)",
				path_1,
				path_2,
		...
		if (!wcscmp(dentries_1[i].cFileName, L".") ||
			...
			continue;
		tree_cmp(path_1,
		         path_1_len + 1 + name_1_len,
		         path_2,
		         path_2_len + 1 + name_2_len);
	}
...
get_security(const wchar_t *path, size_t *len_ret)
{
	...
	if (!GetFileSecurity(
		    path, requestedInformation, descr, lenNeeded, &lenNeeded))
		goto err;
	*len_ret = lenNeeded;
	return descr;
err:
...
static void
cmp_attributes(const wchar_t *path_1,
               const BY_HANDLE_FILE_INFORMATION *file_info_1,
               const wchar_t *path_2,
               const BY_HANDLE_FILE_INFORMATION *file_info_2)
{
	...
	for (size_t i = 0; i < ARRAY_LEN(file_attr_flags); i++) {
		if (differences & file_attr_flags[i].flag) {
			...
			fwprintf(
				stderr,
				L"\t%ls has FILE_ATTRIBUTE_%ls set but %ls does not\n",
				set_path,
...
tree_cmp(wchar_t *path_1, size_t path_1_len, wchar_t *path_2, size_t path_2_len)
{
	...
	if (!GetFileInformationByHandle(hFile_1, &file_info_1))
		fatal_win32_error(L"Failed to get file information for %ls",
		                  path_1);
	...
	common_attribs = file_info_1.dwFileAttributes &
	                 file_info_2.dwFileAttributes;
	...
	if (!(common_attribs & FILE_ATTRIBUTE_DIRECTORY)) {
		...
		if (size_1 != size_2)
			difference(L"Size for %ls (%" PRIu64
			           ") differs from size for %ls (%" PRIu64 ")",
			           path_1,
			           size_1,
			           path_2,
			           size_2);
	...
	/* Compare file times.  */
	if (!file_times_equal(&file_info_1.ftCreationTime,
	                      &file_info_2.ftCreationTime))
		difference(L"Creation times on %ls and %ls differ",
		           path_1,
		           path_2);

	if (!file_times_equal(&file_info_1.ftLastWriteTime,
	                      &file_info_2.ftLastWriteTime))
	...
	if (ino_to == INODE_NOT_SEEN_YET)
		...
		difference(L"%ls and %ls are hard linked differently",
		           path_1,
		           path_2);
...
enable_privilege(const wchar_t *privilege)
{
	...
	if (!LookupPrivilegeValue(NULL, privilege, &luid))
		fatal_win32_error(L"Failed to look up privileges %ls",
		                  privilege);
	...
	if (!AdjustTokenPrivileges(hToken, FALSE, &newState, 0, NULL, NULL))
		fatal_win32_error(L"Failed to acquire privilege %ls",
		                  privilege);
	CloseHandle(hToken);
}
...
wmain(int argc, wchar_t **argv)
{
	...
	if (difference_count) {
		fwprintf(
			stderr,
			L"Found %lu differences; exiting with failure status.\n",
			difference_count);
		return 1;
	}
	return 0;
