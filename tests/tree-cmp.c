...
static u64
do_lookup_ino(struct node *tree, u64 ino_from)
{
	if (!tree)
		return -1;
...
static void
do_insert(struct node *tree, struct node *node)
{
...
static u64
lookup_ino(u64 ino_from)
{
	return do_lookup_ino(tree, ino_from);
}

static void
insert_ino(u64 ino_from, u64 ino_to)
{
...
static void
cmp(const char *file1, const char *file2, size_t size)
{
	int fd1, fd2;
          ...
          bool missingok)
{
	...
	if (lgetxattr(file1, xattr_name, buf1, len1) != len1)
		error("Could not read xattr `%s' of `%s'", xattr_name, file1);

	len2 = lgetxattr(file2, xattr_name, buf2, len1);
	if (len2 == len1) {
		if (memcmp(buf1,
		           buf2,
		           (max_size == 0 || len1 <= max_size) ? len1 :
		                                                 max_size))
		{
...
static void
cmp_ads(const char *file1, const char *file2)
{
	...
	if (len1 == -1) {
		if (errno != ERANGE ||
		    ((len1 = llistxattr(file1, NULL, 0) == -1)))
 ...
 * attributes. */
static void
special_cmp(const char *file1, const char *file2)
{
#ifdef __linux__
...
#else
	fprintf(stderr,
	        "tree-cmp: Warning: cannot compare xattrs of `%s' and `%s'\n",
	        file1,
	        file2);
	fprintf(stderr,
...
static void
tree_cmp(char file1[], int file1_len, char file2[], int file2_len)
{
	struct stat st1, st2;
	u64 ino_from, ino_to;
		...
		for (i = 0; i < ret1; i++) {
			...
			if (!(name[0] == '.' &&
			      (name[1] == '\0' ||
			       (name[1] == '.' && name[2] == '\0'))))
			{
				memcpy(file1 + file1_len + 1,
				       name,
				       name_len + 1);
				memcpy(file2 + file2_len + 1,
				       name,
				       name_len + 1);
				tree_cmp(file1,
				         file1_len + 1 + name_len,
				         file2,
				         file2_len + 1 + name_len);
			}
