...
#define for_opt(c, opts) \
  while ((c = getopt_long_only(argc, (tchar **)argv, T(""), opts, NULL)) != -1)
...
static const tchar *
get_cmd_string(int cmd, bool only_short_form);
...
static void
imagex_suppress_output(void)
{
	imagex_info_file = NULL;
}

static void
imagex_output_to_stderr(void)
{
	if (imagex_info_file)
...
static void
_format_attribute(printf, 1, 2) imagex_error(const tchar *format, ...)
{
	va_list va;
	va_start(va, format);
...
static int
verify_image_exists_and_is_single(int image,
...
get_compression_type(tchar *optarg, bool solid)
{
	...
	if (plevel) {
		...
		*plevel++ = T('\0');
		...
		{
			imagex_error(T(
				"Compression level must be a positive integer! "
				"e.g. --compress=lzx:80"));
	...
	if (!tstrcasecmp(optarg, T("maximum")) ||
	    !tstrcasecmp(optarg, T("lzx")) || !tstrcasecmp(optarg, T("max")))
	{
		ctype = WIMLIB_COMPRESSION_TYPE_LZX;
	} else if (!tstrcasecmp(optarg, T("fast")) ||
	           !tstrcasecmp(optarg, T("xpress")))
	{
		...
		if (!solid) {
			tfprintf(
				stderr,
				T("Warning: use of '--compress=recovery' is discouraged because it behaves\n"
		...
		imagex_error(T("Invalid compression type \"%" TS "\"!"),
		             optarg);
...
set_compact_mode(const tchar *arg, int *extract_flags)
{
	...
	imagex_error(
		T("\"%" TS
	          "\" is not a recognized System Compression format.  The options are:"
	          "\n"
	          "    --compact=xpress4k\n"
	          "    --compact=xpress8k\n"
	          "    --compact=xpress16k\n"
...
{
	return wimlib_reference_resource_files(wim,
	                                       (const tchar **)list->strings,
	                                       list->num_strings,
		...
		*value++ = '\0';
		...
		if (current_value && !tstrcmp(current_value, value)) {
			...
			imagex_printf(T("Setting the %" TS " property of image "
			              ...
			              image,
			              value);
			ret = wimlib_set_image_property(
				wim, image, name, value);
			if (ret)
				return ret;
			any_changes = true;
		}
                              ...
                              const struct wimlib_wim_info *info)
{
	if (info->part_number != 1) {
		imagex_error(T("\"%" TS
		               "\" is not the first part of the split WIM.\n"
...
static bool
parse_source_list_line(tchar *line,
                       size_t len,
...
parse_source_list(tchar **source_list_contents_p,
...
file_get_contents(const tchar *filename, size_t *len_ret)
{
	...
	if (tstat(filename, &stbuf) != 0) {
		imagex_error_with_errno(
			T("Failed to stat the file \"%" TS "\""), filename);
		goto out;
	}
	len = stbuf.st_size;
	...
	if (!fp) {
		imagex_error_with_errno(
			T("Failed to open the file \"%" TS "\""), filename);
		goto out;
	}
...
translate_text_to_tstr(char *text, size_t num_bytes, size_t *num_tchars_ret)
{
	...
	*num_tchars_ret = num_bytes;
	...
	if (num_bytes >= 2 && (((unsigned char)text[0] == 0xff &&
	                        ...
	                        (unsigned char)text[1] == 0x00)))
	{
		...
		text_wstr = win32_mbs_to_wcs(text, num_bytes, &num_wchars);
		free(text);
	}
...
report_scan_progress(const struct wimlib_progress_info_scan *scan, bool done)
{
	    ...
	    cur_count % 128 == 0)
	{
		...
		imagex_printf(T("\r%" PRIu64 " %" TS " scanned (%" PRIu64
		                " files, "
		                "%" PRIu64 " directories)    "),
...
report_split_progress(uint64_t bytes_completed_in_part)
{
	uint64_t completed_bytes =
		last_split_progress.completed_bytes + bytes_completed_in_part;
	unsigned percent_done =
		TO_PERCENT(completed_bytes, last_split_progress.total_bytes);
	unsigned unit_shift;
	const tchar *unit_name;
                     ...
                     void *_ignored_context)
{
	...
	case WIMLIB_PROGRESS_MSG_WRITE_STREAMS:
		...
		}
		{
			static bool started;
			if (!started) {
				if (info->write_streams.compression_type !=
				    WIMLIB_COMPRESSION_TYPE_NONE)
				{
					imagex_printf(
						T("Using %" TS " compression "
					          "with %u thread%" TS "\n"),
		...
		unit_shift =
			get_unit(info->write_streams.total_bytes, &unit_name);
		percent_done = TO_PERCENT(info->write_streams.completed_bytes,
		                          info->write_streams.total_bytes);

		imagex_printf(T("\rArchiving file data: %" PRIu64 " %" TS
		                " of %" PRIu64 " %" TS " (%u%%) done"),
		              ...
		              unit_name,
		              percent_done);
		if (info->write_streams.completed_bytes >=
		    info->write_streams.total_bytes)
			imagex_printf(T("\n"));
		break;
	...
	case WIMLIB_PROGRESS_MSG_VERIFY_INTEGRITY:
		...
		imagex_printf(T("\rVerifying integrity of \"%" TS "\": %" PRIu64
		                " %" TS " "
		                "of %" PRIu64 " %" TS " (%u%%) done"),
		              ...
		              unit_name,
		              percent_done);
		if (info->integrity.completed_bytes ==
		    info->integrity.total_bytes)
			imagex_printf(T("\n"));
		break;
	case WIMLIB_PROGRESS_MSG_CALC_INTEGRITY:
		...
		percent_done = TO_PERCENT(info->integrity.completed_bytes,
		                          info->integrity.total_bytes);
		imagex_printf(
			T("\rCalculating integrity table for WIM: %" PRIu64
		          " %" TS " "
		          "of %" PRIu64 " %" TS " (%u%%) done"),
			info->integrity.completed_bytes >> unit_shift,
			unit_name,
	...
	case WIMLIB_PROGRESS_MSG_EXTRACT_FILE_STRUCTURE:
		if (info->extract.end_file_count >= 2000) {
			percent_done =
				TO_PERCENT(info->extract.current_file_count,
			                   info->extract.end_file_count);
			imagex_printf(T("\rCreating files: %" PRIu64
			                " of %" PRIu64 " (%u%%) done"),
			              info->extract.current_file_count,
			              info->extract.end_file_count,
			              percent_done);
			if (info->extract.current_file_count ==
			    info->extract.end_file_count)
				imagex_printf(T("\n"));
		}
		break;
	...
	case WIMLIB_PROGRESS_MSG_EXTRACT_METADATA:
		if (info->extract.end_file_count >= 2000) {
			percent_done =
				TO_PERCENT(info->extract.current_file_count,
			                   info->extract.end_file_count);
			imagex_printf(T("\rApplying metadata to files: %" PRIu64
			                " of %" PRIu64 " (%u%%) done"),
			              info->extract.current_file_count,
			              info->extract.end_file_count,
			              percent_done);
	...
	case WIMLIB_PROGRESS_MSG_WIMBOOT_EXCLUDE:
		imagex_printf(T("\nExtracting \"%" TS
		                "\" as normal file (not WIMBoot pointer)\n"),
	...
	case WIMLIB_PROGRESS_MSG_UNMOUNT_BEGIN:
		if (info->unmount.mount_flags & WIMLIB_MOUNT_FLAG_READWRITE) {
			if (info->unmount.unmount_flags &
			    WIMLIB_UNMOUNT_FLAG_COMMIT)
			{
				imagex_printf(T("Committing changes to %" TS
				                " (image %d)\n"),
				              info->unmount.mounted_wim,
				              info->unmount.mounted_image);
			} else {
				imagex_printf(T("Discarding changes to %" TS
				                " (image %d)\n"),
				              info->unmount.mounted_wim,
				              info->unmount.mounted_image);
				imagex_printf(T(
	...
	case WIMLIB_PROGRESS_MSG_BEGIN_VERIFY_IMAGE:
		imagex_printf(T("Verifying metadata for image %" PRIu32
		                " of %" PRIu32 "\n"),
	...
	case WIMLIB_PROGRESS_MSG_VERIFY_STREAMS:
		...
		unit_shift =
			get_unit(info->verify_streams.total_bytes, &unit_name);
		imagex_printf(T("\rVerifying file data: "
		                "%" PRIu64 " %" TS " of %" PRIu64 " %" TS
		                " (%u%%) done"),
		              info->verify_streams.completed_bytes >>
		                      unit_shift,
		              unit_name,
		              info->verify_streams.total_bytes >> unit_shift,
		              unit_name,
		              percent_done);
		if (info->verify_streams.completed_bytes ==
		    info->verify_streams.total_bytes)
			imagex_printf(T("\n"));
		break;
	default:
...
parse_num_threads(const tchar *optarg)
{
	...
	if (ul_nthreads >= UINT_MAX || *tmp || tmp == optarg) {
		imagex_error(
			T("Number of threads must be a non-negative integer!"));
		return UINT_MAX;
	} else {
		return ul_nthreads;
	}
...
parse_chunk_size(const tchar *optarg)
{
	...
	if (chunk_size == 0) {
		imagex_error(T(
			"Invalid chunk size specification; must be a positive integer\n"
	...
	if (*tmp) {
		...
		if (*tmp && !(*tmp == T('i') && *(tmp + 1) == T('B'))) {
			imagex_error(T(
				"Invalid chunk size specification; suffix must be K, M, or G"));
			return UINT32_MAX;
		}
	}
	if (chunk_size >= UINT32_MAX) {
		imagex_error(T(
			"Invalid chunk size specification; the value is too large!"));
		return UINT32_MAX;
	}
	return chunk_size;
}
 ...
 */
static bool
update_command_add_option(int op,
                          const tchar *option,
                          struct wimlib_update_command *cmd)
{
	...
	case WIMLIB_UPDATE_OP_ADD:
		...
		else if (!tstrcmp(option, T("--no-acls")) ||
		         !tstrcmp(option, T("--noacls")))
...
static void
update_command_add_nonoption(int op,
                             const tchar *nonoption,
                             struct wimlib_update_command *cmd,
 ...
 */
static bool
parse_update_command(tchar *line,
                     size_t len,
                     ...
                     size_t line_number)
{
	...
	if (!tstrcasecmp(command_name, T("add"))) {
		...
		imagex_error(T("Unknown update command \"%" TS
		               "\" on line %zu"),
		             command_name,
		             line_number);
		return false;
	...
	for (;;) {
		...
		if (next_string[0] == T('-') && next_string[1] == T('-')) {
			...
			{
				imagex_error(T("Unrecognized option \"%" TS
				               "\" to "
				               "update command \"%" TS
				               "\" on line %zu"),
				             next_string,
			...
			update_command_add_nonoption(
				op, next_string, command, num_nonoptions);
			num_nonoptions++;
		}
	}
...
parse_update_command_file(tchar **cmd_file_contents_p,
                          ...
                          size_t *num_cmds_ret)
{
	...
	nlines = text_file_count_lines(cmd_file_contents_p, &cmd_file_nchars);
	if (nlines < 0)
		return NULL;
...
imagex_apply(int argc, tchar **argv, int cmd)
{
	...
	if (!tstrcmp(wimfile, T("-"))) {
		...
		ret = wimlib_open_wim_with_progress(
			wimfile, open_flags, &wim, imagex_progress_func, NULL);
		if (ret)
			goto out_free_refglobs;
		...
		if (argc >= 3) {
			...
			ret   = verify_image_exists(
                                image, image_num_or_name, wimfile);
			if (ret)
	...
	if (refglobs.num_strings) {
		if (wim == NULL) {
			imagex_error(T(
				"Can't specify --ref when applying from stdin!"));
			ret = -1;
			goto out_wimlib_free;
		}
	...
	if (ret == 0) {
		...
		if (wim) {
			do_resource_not_found_warning(
				wimfile, &info, &refglobs);
		} else {
...
imagex_capture_or_append(int argc, tchar **argv, int cmd)
{
	...
	int add_flags  = WIMLIB_ADD_FLAG_EXCLUDE_VERBOSE |
	                WIMLIB_ADD_FLAG_WINCONFIG | WIMLIB_ADD_FLAG_VERBOSE |
	...
	for_opt(c, capture_or_append_options)
	{
		...
		case IMAGEX_FLAGS_OPTION: {
			tchar *p = alloca((6 + tstrlen(optarg) + 1) *
			                  sizeof(tchar));
...
#ifdef _WIN32
			imagex_printf(T(
				"[WARNING] '--update-of' is unreliable on Windows!\n"));
#endif
			break;
		...
		case IMAGEX_CREATE_OPTION:
			if (cmd == CMD_CAPTURE) {
				imagex_error(T(
					"'--create' is only valid for 'wimappend', not 'wimcapture'"));
				goto out_err;
			}
			create = true;
			break;
	...
	if (argc >= 3) {
		...
		source_copy =
			alloca((source_name_len + 1 + 25) * sizeof(tchar));
	...
	if (source_list) {
		...
		if (source[0] == T('-') && source[1] == T('\0')) {
			source_list_contents =
				stdin_get_text_contents(&source_list_nchars);
		} else {
			source_list_contents = file_get_text_contents(
				source, &source_list_nchars);
		}
	...
	if (appending && name_defaulted) {
		...
		for (conflict_idx = 1; wimlib_image_name_in_use(wim, name);
		     conflict_idx++)
		{
	...
	if (base_wimfiles.num_strings) {
		base_wims =
			calloc(base_wimfiles.num_strings, sizeof(base_wims[0]));
		if (base_wims == NULL) {
			imagex_error(T("Out of memory!"));
			ret = -1;
			goto out_free_wim;
		...
		ret = wimlib_reference_resources(
			wim, base_wims, base_wimfiles.num_strings, 0);
		if (ret)
			goto out_free_base_wims;
		...
		if (base_wimfiles.num_strings == 1) {
			...
			imagex_printf(
				T("Capturing delta WIM based on %u WIMs\n"),
	...
	if (template_image_name_or_num) {
		...
		template_image = wimlib_resolve_image(
			template_wim, template_image_name_or_num);
		...
		ret = verify_image_exists_and_is_single(
			template_image,
	...
	ret = wimlib_add_image_multisource(
		wim, capture_sources, num_sources, name, config_file, add_flags);
	if (ret)
		goto out_free_template_wim;
	...
	if (image_properties.num_strings || template_image_name_or_num) {
		...
		ret = apply_image_properties(
			&image_properties, wim, info.image_count, NULL);
		if (ret)
			goto out_free_template_wim;
	...
	if (appending) {
		...
		ret = wimlib_write(wim,
		                   wimfile,
		                   WIMLIB_ALL_IMAGES,
		                   write_flags,
		                   num_threads);
	} else {
		ret = wimlib_write_to_fd(wim,
		                         wim_fd,
		                         WIMLIB_ALL_IMAGES,
		                         write_flags,
		                         num_threads);
...
imagex_delete(int argc, tchar **argv, int cmd)
{
	...
	if (ret) {
		imagex_error(T("Failed to write the file \"%" TS
		               "\" with image "
		               "deleted"),
		             wimfile);
	}
...
static void
print_time(const tchar *type,
           const struct wimlib_timespec *wts,
           int32_t high_part)
{
...
static void
print_byte_field(const uint8_t field[], size_t len)
{
	while (len--)
...
static int
print_resource(const struct wimlib_resource_entry *resource, void *_ignore)
{
	tprintf(T("Hash              = 0x"));
...
print_dentry_detailed(const struct wimlib_dir_entry *dentry)
{
	...
	print_time(T("Creation Time"),
	           ...
	           dentry->creation_time_high);
	print_time(T("Last Write Time"),
	           &dentry->last_write_time,
	           dentry->last_write_time_high);
	print_time(T("Last Access Time"),
	           &dentry->last_access_time,
	           dentry->last_access_time_high);
	...
	if (!is_null_guid(dentry->object_id.object_id)) {
		...
		print_guid(T("Birth Volume ID"),
		           dentry->object_id.birth_volume_id);
		print_guid(T("Birth Object ID"),
		           dentry->object_id.birth_object_id);
...
imagex_dir(int argc, tchar **argv, int cmd)
{
	...
	wimfile = argv[0];
	ret     = wimlib_open_wim_with_progress(
                wimfile, 0, &wim, imagex_progress_func, NULL);
	if (ret)
		goto out_free_refglobs;
	...
	ret = wimlib_iterate_dir_tree(
		wim, image, path, iterate_flags, print_dentry, &options);
...
imagex_export(int argc, tchar **argv, int cmd)
{
	...
	ret                   = wimlib_open_wim_with_progress(
                src_wimfile, open_flags, &src_wim, imagex_progress_func, NULL);
	if (ret)
		goto out_free_refglobs;
	...
	if (dest_wimfile != NULL && tstat(dest_wimfile, &stbuf) == 0) {
		...
		if (compression_type != WIMLIB_COMPRESSION_TYPE_INVALID) {
			...
			if (compression_type != dest_info.compression_type) {
				imagex_error(T(
					"Cannot specify a compression type that is "
		...
		wimlib_register_progress_function(
			dest_wim, imagex_progress_func, NULL);
		           ...
		           chunk_size == UINT32_MAX)
		{
			...
			wimlib_set_output_chunk_size(dest_wim,
			                             src_info.chunk_size);
		}
	}
	...
	if (solid_ctype != WIMLIB_COMPRESSION_TYPE_INVALID) {
		ret = wimlib_set_output_pack_compression_type(dest_wim,
		                                              solid_ctype);
		if (ret)
			goto out_free_dest_wim;
	}
	if (solid_chunk_size != UINT32_MAX) {
		ret = wimlib_set_output_pack_chunk_size(dest_wim,
		                                        solid_chunk_size);
		if (ret)
			goto out_free_dest_wim;
	}
	...
	{
		imagex_error(T(
			"--boot specified for all-images export, but source WIM "
			"has no bootable image."));
		ret = -1;
		goto out_free_dest_wim;
	}

	ret = wimlib_export_image(
		src_wim, image, dest_wim, dest_name, dest_desc, export_flags);
	if (ret) {
		if (ret == WIMLIB_ERR_RESOURCE_NOT_FOUND) {
			do_resource_not_found_warning(
				src_wimfile, &src_info, &refglobs);
	...
	if (!wim_is_new)
		...
		ret = wimlib_write(dest_wim,
		                   dest_wimfile,
		                   WIMLIB_ALL_IMAGES,
		                   write_flags,
		                   num_threads);
	else
		ret = wimlib_write_to_fd(dest_wim,
		                         dest_wim_fd,
		                         WIMLIB_ALL_IMAGES,
		                         write_flags,
		                         num_threads);
...
imagex_extract(int argc, tchar **argv, int cmd)
{
	...
	while (argc != 0 && ret == 0) {
		...
		if (num_paths) {
			...
			ret = wimlib_extract_pathlist(
				wim, image, dest_dir, listfile, extract_flags);
			argc--;
			argv++;
		}
	}
	...
	if (ret == 0) {
		...
		tfprintf(stderr,
		         T("Note: You can use `%" TS "' to see what "
...
imagex_info(int argc, tchar **argv, int cmd)
{
	...
	if (boot && info.image_count == 0) {
		imagex_error(
			T("--boot is meaningless on a WIM with no images"));
		goto out_wimlib_free;
	}
	...
	if (!image_properties.num_strings && !boot) {
		...
		if (blobs) {
			if (info.total_parts != 1) {
				tfprintf(stderr,
				         T("Warning: Only showing the blobs "
		...
		if (boot) {
			if (image == info.boot_index) {
				...
				imagex_printf(
					T("Marking image %d as bootable.\n"),
					image);
				info.boot_index = image;
				ret             = wimlib_set_wim_info(
                                        wim, &info, WIMLIB_CHANGE_BOOT_INDEX);
				if (ret)
					goto out_wimlib_free;
			}
		}
		...
		ret = apply_image_properties(
			&image_properties, wim, image, &any_property_changes);
		if (ret)
			goto out_wimlib_free;
...
imagex_split(int argc, tchar **argv, int cmd)
{
	...
	ret = wimlib_open_wim_with_progress(
		argv[0], open_flags, &wim, imagex_progress_func, NULL);
	if (ret)
		goto out;
...
imagex_unmount(int argc, tchar **argv, int cmd)
{
	...
	ret = wimlib_unmount_image_with_progress(
		argv[0], unmount_flags, imagex_progress_func, NULL);
	if (ret) {
...
imagex_update(int argc, tchar **argv, int cmd)
{
	...
	for_opt(c, update_options)
	{
		...
		case IMAGEX_COMMAND_OPTION:
			if (command_str) {
				imagex_error(
					T("--command may only be specified "
	...
	if (argc >= 2) {
		...
		if (info.image_count != 1) {
			imagex_error(
				T("\"%" TS
			          "\" contains %d images; Please select one."),
				wimfile,
				info.image_count);
			wimlib_free(wim);
			goto out_usage;
	...
	if (command_str) {
		...
		cmds              = parse_update_command_file(
                        &command_str, tstrlen(command_str), &num_cmds);
		if (!cmds) {
			ret = -1;
		...
		cmds = parse_update_command_file(
			&cmd_file_contents, cmd_file_nchars, &num_cmds);
		if (!cmds) {
			ret = -1;
			goto out_free_cmd_file_contents;
		}
...
imagex_verify(int argc, tchar **argv, int cmd)
{
	...
	if (argc != 1) {
		if (argc == 0)
			...
			imagex_error(
				T("At most one WIM file can be specified!"));
		goto out_usage;
	}
	...
	if (ret) {
		tputc(T('\n'), stderr);
		imagex_error(T("\"%" TS "\" failed verification!"), wimfile);
		if (ret == WIMLIB_ERR_RESOURCE_NOT_FOUND &&
		    refglobs.num_strings == 0)
		{
			imagex_printf(T(
				"Note: if this WIM file is not standalone, "
...
static const tchar *const usage_strings[] = {
	...
	[CMD_DIR] =
		T("    %" TS " WIMFILE [IMAGE] [--path=PATH] [--detailed]\n"),
	[CMD_EXPORT] = T(
	...
	[CMD_OPTIMIZE] = T(
		...
		"\n"),
	[CMD_SPLIT] = T("    %" TS
	                " WIMFILE SPLIT_WIM_PART_1 PART_SIZE_MB [--check]\n"),
#if WIM_MOUNTING_SUPPORTED
	[CMD_UNMOUNT] = T(
		"    %" TS " DIRECTORY\n"
...
version(void)
{
	static const tchar *const fmt = T(
...
recommend_man_page(int cmd, FILE *fp)
{
	...
	format_str = T("Some uncommon options are not listed;\n"
	               "See %" TS
	               ".pdf in the doc directory for more details.\n");
#else
	format_str = T("Some uncommon options are not listed; see `man %" TS
	               "' for more details.\n");
#endif
