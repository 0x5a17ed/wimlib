...
{
	return (const ntfschar *)((const u8 *)record +
...
open_ntfs_attr(ntfs_inode *ni, const struct ntfs_location *loc)
{
	...
	na = ntfs_attr_open(
		ni, loc->attr_type, loc->attr_name, loc->attr_name_nchars);
	if (!na) {
		ERROR_WITH_ERRNO(
			"Failed to open attribute of NTFS inode %" PRIu64,
			loc->mft_no);
	}
	return na;
}
                            ...
                            struct ntfs_volume_wrapper *volume)
{
	if (inode->i_reparse_tag == WIM_IO_REPARSE_TAG_DEDUP &&
	    ...
	    !volume->dedup_warned)
	{
		WARNING("Filesystem includes files deduplicated with Windows'\n"
                          ...
                          ATTR_TYPES type)
{
	...
	while (!ntfs_attr_lookup(
		type, NULL, 0, CASE_SENSITIVE, 0, NULL, 0, actx))
	{
	...
	if (errno != ENOENT) {
		ERROR_WITH_ERRNO("Error listing NTFS attributes of \"%s\"",
		                 path);
		ret = WIMLIB_ERR_NTFS_3G;
		goto out_put_actx;
	}
...
static noinline_for_stack int
get_security_descriptor(ntfs_inode *ni,
...
static int
insert_dos_name(struct dos_name_map *map,
                const ntfschar *dos_name,
                size_t name_nbytes,
                u64 ntfs_ino)
{
	...
	if (avl_tree_insert(
		    &map->root, &new_node->index_node, _avl_cmp_by_ntfs_ino))
	{
		...
		WARNING("NTFS inode %" PRIu64 " has multiple DOS names",
		        ntfs_ino);
		FREE(new_node);
	}
	return 0;
...
lookup_dos_name(const struct dos_name_map *map, u64 ntfs_ino)
{
	...
	res = avl_tree_lookup_node(
		map->root, &dummy.index_node, _avl_cmp_by_ntfs_ino);
	if (!res)
		return NULL;
...
set_dentry_dos_name(struct wim_dentry *dentry, const struct dos_name_map *map)
{
	...
	if (dentry->d_is_win32_name) {
		...
		if (node) {
			dentry->d_short_name =
				utf16le_dupz(node->dos_name, node->name_nbytes);
...
destroy_dos_name_map(struct dos_name_map *map)
{
	...
	avl_tree_for_each_in_postorder(
		node, map->root, struct dos_name_node, index_node)
		FREE(node);
}
        ...
        const unsigned dt_type)
{
	...
	if (name_type & FILE_NAME_DOS) {
		...
		ret = insert_dos_name(
			&ctx->dos_name_map, name, name_nbytes, MREF(mref));
	...
	if (!pathbuf_append_name(
		    params, mbs_name, mbs_name_nbytes, &orig_path_nchars))
		goto out_free_mbs_name;

	child = NULL;
	ret   = ntfs_3g_build_dentry_tree_recursive(
                &child, mref, mbs_name, name_type, ctx->volume, params);
...
static int
ntfs_3g_recurse_directory(ntfs_inode *ni,
                          struct wim_dentry *parent,
                          struct ntfs_volume_wrapper *volume,
                          struct scan_params *params)
{
                                    ...
                                    struct scan_params *params)
{
	...
	ret = inode_table_new_dentry(
		params->inode_table, filename, ni->mft_no, 0, false, &root);
	if (ret)
		goto out;

	if (name_type &
	    FILE_NAME_WIN32) /* Win32 or Win32+DOS name (rather than POSIX) */
	...
	if (attributes & FILE_ATTRIBUTE_REPARSE_POINT) {
		...
		ret = scan_ntfs_attrs_with_type(inode,
		                                ni,
		                                path,
	 ...
	 * streams.  */
	ret = scan_ntfs_attrs_with_type(
		inode, ni, path, params->unhashed_blobs, volume, AT_DATA);
	if (ret)
		goto out;
	...
	if (!(params->add_flags & WIMLIB_ADD_FLAG_NO_ACLS)) {
		ret = get_security_descriptor(
			ni, inode, volume->vol, params->sd_set);
		if (ret) {
                          ...
                          struct scan_params *params)
{
	...
	ret = ntfs_3g_build_dentry_tree_recursive(
		root_ret, FILE_root, "", FILE_NAME_POSIX, volume, params);
out_close_secure:
