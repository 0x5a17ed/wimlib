...
query_device(HANDLE h, DWORD code, void *out, DWORD out_size)
{
	...
	return DeviceIoControl(
		h, code, NULL, 0, out, out_size, &bytes_returned, NULL);
}
	...
	*(wcschr(vol_name, L'X')) = path[0];
	...
	if (extents->NumberOfDiskExtents != 1) {
		ERROR("\"%ls\": This volume has %" PRIu32 " disk extents, "
		      ...
		      vol_name,
		      (u32)extents->NumberOfDiskExtents);
		ret = WIMLIB_ERR_UNSUPPORTED;
		goto out;
	}

	wsprintf(wcschr(disk_name, L'X'),
	         L"%" PRIu32,
...
write_wimoverlay_dat(const wchar_t *path, const void *contents, u32 size)
{
	...
	if (h == INVALID_HANDLE_VALUE) {
		win32_error(GetLastError(),
		            L"\"%ls\": Can't open file for writing",
		            path);
	...
	if (!WriteFile(h, contents, size, &bytes_written, NULL) ||
	    bytes_written != size)
	{
		win32_error(GetLastError(), L"\"%ls\": Can't write file", path);
		CloseHandle(h);
	...
	if (!CloseHandle(h)) {
		win32_error(
			GetLastError(), L"\"%ls\": Can't close handle", path);
		return WIMLIB_ERR_WRITE;
	}
                       ...
                       u32 new_entry_2_size)
{
	...
	new_hdr->next_data_source_id =
		(old_hdr ? old_hdr->next_data_source_id : 0) + 1;
	...
	if (old_hdr) {
		for (u32 i = 0; i < old_hdr->num_entries; i++) {
			wimlib_assert(new_hdr->entry_1s[i].entry_2_offset ==
			              p - buf);
			wimlib_assert(old_hdr->entry_1s[i].entry_2_length ==
			              new_hdr->entry_1s[i].entry_2_length);
			p = mempcpy(p,
			            ((const u8 *)old_hdr +
			             old_hdr->entry_1s[i].entry_2_offset),
	...
	if (part_info->PartitionStyle == PARTITION_STYLE_MBR) {
		new_entry_2->partition.mbr.part_start_offset =
			part_info->StartingOffset.QuadPart;
		new_entry_2->partition.mbr.padding = 0;
		new_entry_2->partition_table_type =
			WIMOVERLAY_PARTITION_TYPE_MBR;
		...
		STATIC_ASSERT(
			sizeof(new_entry_2->disk.gpt.disk_guid) ==
			sizeof(new_entry_2->partition.gpt.part_unique_guid));
	}
                       ...
                       u64 *new_data_source_id_ret)
{
	...
	ret = calculate_wimoverlay_dat_size(
		old_hdr, new_entry_2_size, &new_contents_size);
	if (ret)
		return ret;
	...
	end = fill_in_wimoverlay_dat(buf,
	                             old_hdr,
	                             wim_path,
	                             wim_guid,
	                             image,
...
retry:
	...
	if (h == INVALID_HANDLE_VALUE) {
		    ...
		    func_RtlCreateSystemVolumeInformationFolder)
		{
			...
			*(wcschr(volume_root_path, L'X')) = path[0];
			...
			UNICODE_STRING str = {
				.Length = sizeof(volume_root_path) -
				          sizeof(wchar_t),
			...
			status = (*func_RtlCreateSystemVolumeInformationFolder)(
				&str);
	...
	if (!GetFileInformationByHandle(h, &info)) {
		win32_error(
			GetLastError(), L"\"%ls\": Can't query metadata", path);
		CloseHandle(h);
	...
	if (info.nFileSizeLow < sizeof(struct WimOverlay_dat_header)) {
		ERROR("\"%ls\": File is unexpectedly small (only %" PRIu32
		      " bytes)",
		      path,
		      (u32)info.nFileSizeLow);
		ret = WIMLIB_ERR_UNSUPPORTED;
		goto out_free_contents;
	}
	...
	for (u32 i = 0; i < hdr->num_entries; i++) {
		...
		if (entry_1->data_source_id >= hdr->next_data_source_id) {
			ERROR("\"%ls\": value of next_data_source_id "
			      ...
			      path,
			      hdr->next_data_source_id,
			      i,
			      entry_1->data_source_id);
			ret = WIMLIB_ERR_UNSUPPORTED;
			goto out_free_contents;
		}

		if (((u64)entry_1->entry_2_offset +
		     (u64)entry_1->entry_2_length) > info.nFileSizeLow)
		{
		...
		entry_2 =
			(const struct WimOverlay_dat_entry_2
		...
		if (!valid_wim_filename(entry_2, wim_file_name_length)) {
			...
			if (wimlib_print_errors) {
				print_byte_field(
					(const u8 *)entry_2->wim_file_name,
		...
		if (entry_2->unknown_0x00 != 0x00000000 ||
		    ...
		    entry_2->inner_struct_size !=
		            entry_1->entry_2_length - 0x14 ||
 ...
 */
static int
update_wimoverlay_manually(const wchar_t *drive,
                           const wchar_t *wim_path,
                           const u8 wim_guid[GUID_SIZE],
                           int image,
                           u64 *data_source_id_ret)
{
	...
	wchar_t path_wimlib_backup[] =
		L"A:\\System Volume Information\\WimOverlay.wimlib_backup";
	wchar_t path_new[] =
		L"A:\\System Volume Information\\WimOverlay.wimlib_new";
	...
	wimlib_assert(drive[0] != L'\0' && drive[1] == L':' &&
	              drive[2] == L'\0');
	...
	ret = prepare_wimoverlay_dat(old_contents,
	                             wim_path,
	                             wim_guid,
	                             image,
	                             &new_contents,
	...
	ret = write_wimoverlay_dat(path_new, new_contents, new_contents_size);
	if (ret)
		goto out_free_new_contents;

	/* Write WimOverlay.backup  */
	ret = write_wimoverlay_dat(
		path_backup, new_contents, new_contents_size);
	if (ret)
		goto out_free_new_contents;
	...
	if (ret) {
		ERROR_WITH_ERRNO(
			"Can't rename \"%ls\" => \"%ls\"", path_new, path_main);
		ret = WIMLIB_ERR_RENAME;
	}
                             ...
                             bool *wof_running_ret)
{
	...
	wim_file_name_length =
		sizeof(wchar_t) * (wim_path_nchars + prefix_nchars);
...
retry_ioctl:
	                     ...
	                     NULL))
	{
		...
		if (err == ERROR_INVALID_FUNCTION) {
			...
			win32_error(err,
			            L"Failed to add overlay source \"%ls\" "
			            "to volume \"%ls\"",
			            wim_path,
			            drive_path + 4);
...
out:
	if (ret == WIMLIB_ERR_UNSUPPORTED) {
		...
		ret              = update_wimoverlay_manually(drive_path + 4,
                                                 wim_path,
                                                 wim_guid,
                                                 image,
                                                 data_source_id_ret);
...
retry:
		...
		STATIC_ASSERT(sizeof(in) ==
		              8 + sizeof(WOF_EXTERNAL_INFO) +
		...
		in.wim_info.unnamed_data_stream_size_in_wim =
			blob->rdesc->size_in_wim;
		in.wim_info.unnamed_data_stream_offset_in_wim =
