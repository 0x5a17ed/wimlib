...
struct lzx_compressor {
	...
	void (*impl)(struct lzx_compressor *,
	             const u8 *,
	             size_t,
	             struct lzx_output_bitstream *);
	...
	struct lzx_sequence chosen_sequences[DIV_ROUND_UP(SOFT_MAX_BLOCK_SIZE,
	                                                  LZX_MIN_MATCH_LEN) +
	                                     1];
	...
	union {
		...
		struct {
			 ...
			 */
			struct lzx_optimum_node
				optimum_nodes[SOFT_MAX_BLOCK_SIZE - 1 +
			                      LZX_MAX_MATCH_LEN + 1];
...
lzx_get_offset_slot(struct lzx_compressor *c,
                    u32 adjusted_offset,
                    bool is_16_bit)
{
 ...
 */
static forceinline unsigned
lzx_tally_main_and_lensyms(struct lzx_compressor *c,
                           unsigned length,
                           u32 adjusted_offset,
                           bool is_16_bit)
{
...
lzx_flush_bits(struct lzx_output_bitstream *os, unsigned max_num_bits)
{
	...
	if (os->end - os->next < 6)
		return;
	put_unaligned_le16(os->bitbuf >> ((os->bitcount - 16) & shift_mask),
	                   os->next + 0);
	if (max_num_bits > 16)
		put_unaligned_le16(os->bitbuf >>
		                           ((os->bitcount - 32) & shift_mask),
		                   os->next + 2);
	if (max_num_bits > 32)
		put_unaligned_le16(os->bitbuf >>
		                           ((os->bitcount - 48) & shift_mask),
		                   os->next + 4);
                          ...
                          unsigned num_lens)
{
	...
	*(u8 *)(lens + num_lens) = 0x80;
	...
	num_precode_items = lzx_compute_precode_items(
		lens, prev_lens, precode_freqs, precode_items);
 ...
 */
static void
lzx_write_sequences(struct lzx_output_bitstream *os,
                    int block_type,
                    const u8 *block_data,
                    ...
                    const struct lzx_codes *codes)
{
	...
	for (;;) {
		...
		u32 litrunlen = seq->litrunlen_and_matchlen >>
		                SEQ_MATCHLEN_BITS;
		unsigned matchlen = seq->litrunlen_and_matchlen &
		                    SEQ_MATCHLEN_MASK;
		...
		if (litrunlen) { /* Is the literal run nonempty?  */
			...
			if (CAN_BUFFER(3 * MAIN_CODEWORD_LIMIT)) {
				...
				while (litrunlen >= 3) {
					...
					lzx_add_bits(
						os,
						codes->codewords.main[lit0],
						codes->lens.main[lit0]);
					lzx_add_bits(
						os,
						codes->codewords.main[lit1],
						codes->lens.main[lit1]);
					lzx_add_bits(
						os,
						codes->codewords.main[lit2],
						codes->lens.main[lit2]);
					lzx_flush_bits(os,
					               3 * MAIN_CODEWORD_LIMIT);
					block_data += 3;
					litrunlen -= 3;
				}
				if (litrunlen--) {
					...
					if (litrunlen--) {
						...
						lzx_flush_bits(
							os,
							2 * MAIN_CODEWORD_LIMIT);
					} else {
						lzx_flush_bits(
							os,
							1 * MAIN_CODEWORD_LIMIT);
					}
				}
			} else {
		...
		adjusted_offset = seq->adjusted_offset_and_mainsym >>
		                  SEQ_MAINSYM_BITS;
		main_symbol = seq->adjusted_offset_and_mainsym &
		              SEQ_MAINSYM_MASK;

		offset_slot =
			(main_symbol - LZX_NUM_CHARS) / LZX_NUM_LEN_HEADERS;
		num_extra_bits = lzx_extra_offset_bits[offset_slot];
		extra_bits =
			adjusted_offset - (lzx_offset_slot_base[offset_slot] +
		                           LZX_OFFSET_ADJUSTMENT);

#define MAX_MATCH_BITS                                                    \
  (MAIN_CODEWORD_LIMIT + LENGTH_CODEWORD_LIMIT + LZX_MAX_NUM_EXTRA_BITS - \
   LZX_NUM_ALIGNED_OFFSET_BITS + ALIGNED_CODEWORD_LIMIT)
		...
		lzx_add_bits(os,
		             codes->codewords.main[main_symbol],
		...
		if (matchlen >= LZX_MIN_SECONDARY_LEN) {
			lzx_add_bits(
				os,
				codes->codewords
					.len[matchlen - LZX_MIN_SECONDARY_LEN],
				codes->lens
					.len[matchlen - LZX_MIN_SECONDARY_LEN]);
			if (!CAN_BUFFER(MAX_MATCH_BITS))
		...
		if (offset_slot >= min_aligned_offset_slot) {
			lzx_add_bits(os,
			             extra_bits >> LZX_NUM_ALIGNED_OFFSET_BITS,
			             num_extra_bits -
                           ...
                           struct lzx_output_bitstream *os)
{
	...
	if (block_type == LZX_BLOCKTYPE_ALIGNED) {
		for (int i = 0; i < LZX_ALIGNEDCODE_NUM_SYMBOLS; i++) {
			lzx_write_bits(os,
			               codes->lens.aligned[i],
			               LZX_ALIGNEDCODE_ELEMENT_SIZE);
		}
	}

	/* Output the main code (two parts).  */
	lzx_write_compressed_code(
		os, codes->lens.main, prev_lens->main, LZX_NUM_CHARS);
	lzx_write_compressed_code(os,
	...
	lzx_write_compressed_code(
		os, codes->lens.len, prev_lens->len, LZX_LENCODE_NUM_SYMBOLS);
                               ...
                               const struct lzx_codes *codes)
{
	...
	for (unsigned i = 0; i < LZX_ALIGNEDCODE_NUM_SYMBOLS; i++) {
		verbatim_cost +=
			LZX_NUM_ALIGNED_OFFSET_BITS * freqs->aligned[i];
	 ...
	 * offset code.  */
	aligned_cost +=
		LZX_ALIGNEDCODE_ELEMENT_SIZE * LZX_ALIGNEDCODE_NUM_SYMBOLS;
 ...
 */
static void
lzx_flush_block(struct lzx_compressor *c,
                struct lzx_output_bitstream *os,
...
lzx_should_end_block(struct lzx_block_split_stats *stats)
{
	if (stats->num_observations > 0) {
		...
		if (total_delta >= stats->num_new_observations * 7 / 8 *
		                           stats->num_observations)
			return true;
	}
...
#define LZX_QUEUE_INITIALIZER                                         \
  {                                                                   \
    ((u64)1 << LZX_QUEUE_R0_SHIFT) | ((u64)1 << LZX_QUEUE_R1_SHIFT) | \
...
lzx_lru_queue_swap(struct lzx_lru_queue queue, unsigned idx)
{
	...
	return (struct lzx_lru_queue){ (queue.R & ~(mask | mask_high)) |
		                       ((queue.R & mask_high) >> shift) |
		                       ((queue.R & mask) << shift) };
}

static forceinline u32
lzx_walk_item_list(struct lzx_compressor *c,
                   u32 block_size,
                   bool is_16_bit,
                   bool record)
{
	...
	for (;;) {
		...
		if (item & OPTIMUM_GAP_MATCH) {
			...
			mainsym  = lzx_tally_main_and_lensyms(
                                c, matchlen, 0, is_16_bit);
			if (record) {
		...
		mainsym         = lzx_tally_main_and_lensyms(
                        c, matchlen, adjusted_offset, is_16_bit);
		if (adjusted_offset >=
		    LZX_MIN_ALIGNED_OFFSET + LZX_OFFSET_ADJUSTMENT)
			c->freqs.aligned[adjusted_offset &
			                 LZX_ALIGNED_OFFSET_BITMASK]++;
		if (record) {
			seq->litrunlen_and_matchlen |= (litrun_end - node_idx)
			                               << SEQ_MATCHLEN_BITS;
			seq--;
			seq->litrunlen_and_matchlen = matchlen;
...
#define QUEUE(node)                               \
  (*(struct lzx_lru_queue *)((char *)queues +     \
	                     ((uintptr_t)(node) % \
	                      (ARRAY_LEN(queues) * sizeof(queues[0])))))
...
#  define MATCH_BEFORE_GAP(node)                              \
	...
	do { /* For each node in the block in position order... */
		...
		if (num_matches) {
			...
			unsigned max_len =
				min(block_end - in_next, LZX_MAX_MATCH_LEN);
			const u8 *matchptr;

			/* Consider rep0 matches. */
			matchptr = in_next - lzx_lru_queue_R0(QUEUE(cur_node));
			if (load_u16_unaligned(matchptr) !=
...
rep2_done:
			...
			for (;;) {
				...
				unsigned offset_slot = lzx_get_offset_slot(
					c, adjusted_offset, is_16_bit);
				...
				if (++cache_ptr == end_matches) {
					 ...
					 * + lit + rep0. */
					s32 remaining = (block_end - in_next) -
					                (s32)next_len;
...
done_matches:
		...
		if (cost <= cur_node->cost) {
...
#if CONSIDER_GAP_MATCHES
			s32 adjusted_offset = (s32)cur_node->item >>
			                      OPTIMUM_OFFSET_SHIFT;
			STATIC_ASSERT(OPTIMUM_GAP_MATCH ==
			              0x80000000); /* assuming sign extension */
#else
			u32 adjusted_offset = cur_node->item >>
			                      OPTIMUM_OFFSET_SHIFT;
#endif

			if (adjusted_offset >= LZX_NUM_RECENT_OFFSETS) {
				/* Explicit offset match: insert offset at front. */
				QUEUE(cur_node) = lzx_lru_queue_push(
					QUEUE(cur_node - len),
					adjusted_offset -
						LZX_OFFSET_ADJUSTMENT);
			}
			...
			else if (adjusted_offset < 0)
			{
				 ...
				 * of the queue. */
				u32 match_before_gap =
					MATCH_BEFORE_GAP(cur_node);
				(cur_node - 1)->cost = match_before_gap;
				QUEUE(cur_node)      = lzx_lru_queue_push(
                                        QUEUE(cur_node - len - 1 -
                                              (match_before_gap &
                                               OPTIMUM_LEN_MASK)),
                                        (match_before_gap >>
                                         OPTIMUM_OFFSET_SHIFT) -
			...
			else
			{
				...
				QUEUE(cur_node) = lzx_lru_queue_swap(
					QUEUE(cur_node - len), adjusted_offset);
			}
		}
...
lzx_compute_match_costs(struct lzx_compressor *c)
{
	unsigned num_offset_slots =
		(c->num_main_syms - LZX_NUM_CHARS) / LZX_NUM_LEN_HEADERS;
...
static const u8 literal_scaled_probs[257] = {
	...
	103, 102, 101, 100, 98,  97,  96,  95,  94,  93,  92,  91,  90,  89,
	88,  87,  86,  86,  85,  84,  83,  82,  81,  80,  79,  78,  78,  77,
	76,  75,  74,  73,  73,  72,  71,  70,  70,  69,  68,  67,  67,  66,
	65,  65,  64,  63,  62,  62,  61,  60,  60,  59,  59,  58,  57,  57,
	56,  55,  55,  54,  54,  53,  53,  52,  51,  51,  50,  50,  49,  49,
	48,  48,  47,  47,  46,  46,  45,  45,  44,  44,  43,  43,  42,  42,
	41,  41,  40,  40,  40,  39,  39,  38,  38,  38,  37,  37,  36,  36,
	36,  35,  35,  34,  34,  34,  33,  33,  33,  32,  32,  32,  31,  31,
	31,  30,  30,  30,  29,  29,  29,  28,  28,  28,  27,  27,  27,  27,
	26,  26,  26,  25,  25,  25,  25,  24,  24,  24,  24,  23,  23,  23,
	23,  22,  22,  22,  22,  21,  21,  21,  21,  20,  20,  20,  20,  20,
	19,  19,  19,  19,  19,  18,  18,  18,  18,  18,  17,  17,  17,  17,
	17,  16,  16,  16,  16
};
...
lzx_set_default_costs(struct lzx_compressor *c)
{
	...
	base_literal_prob =
		literal_scaled_probs[num_used_literals] * (1.0f / 6870.0f);
	...
	match_cost = lzx_cost_for_probability(
		prob_match / (c->num_main_syms - LZX_NUM_CHARS));
	...
	for (i = 0; i < LZX_ALIGNEDCODE_NUM_SYMBOLS; i++) {
		...
		unsigned j = (i - LZX_OFFSET_ADJUSTMENT) &
		             LZX_ALIGNED_OFFSET_BITMASK;
...
lzx_set_costs_from_codes(struct lzx_compressor *c)
{
	...
	for (i = 0; i < c->num_main_syms; i++) {
		c->costs.main[i] =
			(lens->main[i] ? lens->main[i] : MAIN_CODEWORD_LIMIT) *
			BIT_COST;
	}

	for (i = 0; i < LZX_LENCODE_NUM_SYMBOLS; i++) {
		c->costs.len[i] =
			(lens->len[i] ? lens->len[i] : LENGTH_CODEWORD_LIMIT) *
			BIT_COST;
	}

#if CONSIDER_ALIGNED_COSTS
	for (i = 0; i < LZX_ALIGNEDCODE_NUM_SYMBOLS; i++) {
		c->costs.aligned[i] = (lens->aligned[i] ?
		                               lens->aligned[i] :
		                               ALIGNED_CODEWORD_LIMIT) *
		                      BIT_COST;
	}
                             ...
                             bool is_16_bit)
{
	...
	for (;;) {
		...
		new_queue = lzx_find_min_cost_path(
			c, block_begin, block_size, initial_queue, is_16_bit);
...
pause:
		...
		next_pause_point = min(
			in_next +
				min(NUM_OBSERVATIONS_PER_BLOCK_CHECK * 2 -
...
end_block:
		...
		queue = lzx_optimize_and_flush_block(c,
		                                     os,
		                                     in_block_begin,
 ...
 */
static forceinline void
lzx_choose_match(struct lzx_compressor *c,
                 unsigned length,
                 u32 adjusted_offset,
                 u32 recent_offsets[LZX_NUM_RECENT_OFFSETS],
                 bool is_16_bit,
                 u32 *litrunlen_p,
                 struct lzx_sequence **next_seq_p)
{
	...
	mainsym = lzx_tally_main_and_lensyms(
		c, length, adjusted_offset, is_16_bit);
	next_seq->litrunlen_and_matchlen = (*litrunlen_p << SEQ_MATCHLEN_BITS) |
	                                   length;
                  ...
                  bool is_16_bit)
{
	...
	do {
		...
		do {
			 ...
			 * offset match. */
			cur_adjusted_offset =
				cur_offset + LZX_OFFSET_ADJUSTMENT;
			cur_score = lzx_explicit_offset_match_score(
				cur_len, cur_adjusted_offset);
			...
			best_rep_len = lzx_find_longest_repeat_offset_match(
				in_next, recent_offsets, max_len, &best_rep_idx);
			in_next++;

			if (best_rep_len != 0 &&
			    (rep_score = lzx_repeat_offset_match_score(
				     best_rep_len, best_rep_idx)) >= cur_score)
			{
...
have_cur_match:
			...
			next_adjusted_offset =
				next_offset + LZX_OFFSET_ADJUSTMENT;
			next_score = lzx_explicit_offset_match_score(
				next_len, next_adjusted_offset);
			...
			if (best_rep_len != 0 &&
			    (rep_score = lzx_repeat_offset_match_score(
				     best_rep_len, best_rep_idx)) >= next_score)
			{
...
choose_cur_match:
		...
		lzx_flush_block(
			c, os, in_block_begin, in_next - in_block_begin, 0);
...
static void
lzx_compress_lazy_16(struct lzx_compressor *c,
                     const u8 *in,
                     size_t in_nbytes,
                     struct lzx_output_bitstream *os)
{
...
static void
lzx_compress_lazy_32(struct lzx_compressor *c,
                     const u8 *in,
                     size_t in_nbytes,
                     struct lzx_output_bitstream *os)
{
...
lzx_init_offset_slot_tabs(struct lzx_compressor *c)
{
	...
	{
		if (adjusted_offset >=
		    lzx_offset_slot_base[slot + 1] + LZX_OFFSET_ADJUSTMENT)
			slot++;
...
static u64
lzx_get_needed_memory(size_t max_bufsize,
...
static int
lzx_create_compressor(size_t max_bufsize,
                      unsigned compression_level,
                      bool destructive,
                      void **c_ret)
{
...
static size_t
lzx_compress(const void *restrict in,
             size_t in_nbytes,
             void *restrict out,
