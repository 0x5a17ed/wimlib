                          ...
                          const struct blob_table *template_blob_table)
{
	...
	for (unsigned i = 0; i < inode->i_num_streams; i++) {
		...
		template_strm = inode_get_stream(
			template_inode, strm->stream_type, strm->stream_name);
		if (!template_strm)
			return false;
                     ...
                     struct blob_table *template_blob_table)
{
	for (unsigned i = 0; i < inode->i_num_streams; i++) {
		 ...
		 * hashed.  */
		if (!blob || !template_blob || !blob->unhashed ||
		    template_blob->unhashed)
			continue;
...
static int
reference_template_file(struct wim_inode *inode,
                        WIMStruct *wim,
                        WIMStruct *template_wim)
{
	...
	template_dentry = get_dentry(
		template_wim, dentry->d_full_path, WIMLIB_CASE_SENSITIVE);
	if (template_dentry != NULL &&
	    inode_metadata_consistent(inode,
	                              template_dentry->d_inode,
	                              wim->blob_table,
	                              template_wim->blob_table))
	{
		inode_copy_checksums(inode,
		                     template_dentry->d_inode,
		                     wim->blob_table,
		                     template_wim->blob_table);
	}
